...
int equal(CAR c1,CAR c2,int condition)  /*如何判断两个CAR记录相等*/
{
	...
	else if (condition==2)                /*如果参数condition的值为2，则比较型号*/
     {
	     if (strcmp(c1.name,c2.name)==0) 	return 1;
		else return 0;
     }

 else if (condition==3)                /*如果参数condition的值为3，则比较差价*/
		return c1.p3==c2.p3;
 else if (condition==4)
...
void calcuRank(CAR c[],int n)            /*根据差价计算所以汽车的排名，差价相同这名次相同*/
{int i;
...
{
if (equal(c[i],c[i-1],3))                /*如果当前记录与其相邻的前一条记录总分相等*/
...
void printrank(CAR c[],int n)             /*输出差价排序后所以汽车记录的值*/
{  int i;
	sortC(c,50,4);
...
int insertC(CAR c[],int n,CAR ca)              /*向stu数组中依编号递增插入一个元素s*/
{
	int i;
	sortC(c,n,1);                              /*先按编号排序*/
	for (i=0;i<n;i++)
	...
	{
		if (!larger(c[i],ca,1))                    /*如果s大于当前元素c[i]，则退出循环*/
		break;
