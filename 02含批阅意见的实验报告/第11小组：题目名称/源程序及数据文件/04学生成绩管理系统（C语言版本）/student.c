...
int readCus(Custome  *Cus , int n)          /*读入学生记录值，学号为0或读满规定条数记录时停止*/
{
	int i,j;
	for (i=0;i<n;i++)
	{
		printf("Input one Custome\'s information\n");
		printf("num:  ");
	     scanf("%ld", &Cus[i].num);    //桌号 
		if (Cus[i].num==0) break;
		printf("name: ");              //姓氏 
		scanf("%s",Cus[i].name);	
		printf("sex:  ");               //性别 
		scanf("%s",Cus[i].sex);
